...
server <- function(input, output, session) {
  ...
  # Conditional rendering of the Chemistry Download Button
  output$download_chemistry_ui <- renderUI({
    if (data_storage$data_loaded) {
      downloadButton("download_chemistry", "Download Water Chemistry Report(PDF)")
    } else {
      NULL # Or display a message like "Load data to enable download"
    }
  })
  ...
  output$chemCompPlot <- renderPlot({
    ...
    avg_comp <- colMeans(PD[, c("Ca_pct", "Mg_pct", "Na_pct", "Cl_pct", "SO4_pct", "HCO3_pct")], na.rm = TRUE)
    
    barplot(
      avg_comp,
      col = rainbow(6),
      main = "Average Chemical Composition (%)",
      ylab = "Percentage",
      ylim = c(0, 100),
  ...
  output$measures_section <- renderUI({
    ...
    types <- unique(PD$water_type)
    measures_list <- list(
      "Permanent Hardness" = c("- Requires investment in softening infrastructure. (Drinking water)", "- May reduce soil permeability. (Agriculture)", "- Accelerates equipment corrosion. (Industrial)", "- Elevated sulfate affects aquatic life. (Environment)", "- Calcium intake consideration. (Health)"),
      "Temporary Hardness" = c("- Monitor in real-time. (Drinking water)", "- Improves soil structure. (Agriculture)", "- Descaling costs. (Industrial)", "- Alkaline boosts aquatic life. (Environment)", "- Magnesium levels check. (Health)"),
      "Alkali Carbonates" = c("- Desalination may be required. (Drinking water)", "- Sodicity management. (Agriculture)", "- Scaling in industry. (Industrial)", "- Alters pH in ecosystems. (Environment)", "- Sodium control strategies. (Health)"),
      "Saline" = c("- Desalination needed. (Drinking water)", "- Leaching required. (Agriculture)", "- High corrosion. (Industrial)", "- Recharge policy. (Environment)", "- Monitor hypertension risk. (Health)"),
      "Mixed Type" = c("- Needs integrated management across sectors."),
      "Unable to compute" = c("- Insufficient data.")
    )
    
    measures_html <- lapply(types, function(type) {
      if (type %in% names(measures_list)) {
        paste("<b>", type, " — Suggested Measures:</b><br>", paste(measures_list[[type]], collapse = "<br>"))
      } else {
        paste("<b>", type, " — No measures defined.</b>")
      }
    })
    
    HTML(paste(measures_html, collapse = "<br><br>"))
  })
